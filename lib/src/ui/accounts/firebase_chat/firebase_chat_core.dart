...
class FirebaseChatCore {
  ...
  /// rooms list
  Future<void> createUserInFirestore(types.User user, int wpUserId) async {
  ...
  Stream<List<types.Room>> rooms(showDialogOnError, {bool orderByUpdatedAt = false}) {
    final fu = firebaseUser;
    ...
    return collection.snapshots().handleError((e) {showDialogOnError(e); print(e);}).asyncMap(
  ...
  Stream<List<types.User>> users() {
    ...
    return getFirebaseFirestore()
        .collection(config.usersCollectionName)
        .orderBy("updatedAt", descending: true)
        .snapshots().handleError((e) {print(e);})
        .map(
  ...
  /// Returns a stream of all users from Firebase
  Future<types.User?> user(String id) async {
    if (firebaseUser == null) return null;
    final docRef = getFirebaseFirestore()
        .collection(config.usersCollectionName)
        .doc(id);
